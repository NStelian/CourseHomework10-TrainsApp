...
public class TrainDao {
    ...
    public List<Train> getAll(TrainFilters filters) {
        ...
        List<Location> locations = getLocations(locationCriteria);
        return filterTrains(trains, locations);
    }
    ...
    private Criteria getTrainCriteria(TrainFilters filters) {
        Criteria trainsCriteria = new Criteria();
        ofNullable(filters.getTrainModel())
                .ifPresent(trainModel -> trainsCriteria.and("model").is(trainModel));
        ofNullable(filters.getMinCarts())
                .ifPresent(minCarts -> trainsCriteria.and("carts").gt(minCarts));
        ofNullable(filters.getMaxCarts())
                .ifPresent(maxCarts -> trainsCriteria.and("carts").lt(maxCarts));
        return trainsCriteria;
    }
}
